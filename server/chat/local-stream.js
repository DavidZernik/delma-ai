...
function buildLocalSystemPrompt({ projectDir, claudeMdRaw }) {
  ...
  lines.push(`Project-specific knowledge that wouldn't apply to other projects. Goals, defining decisions, open todos, and the system flow diagram all live here.`)
  lines.push(``)
  lines.push(`Goes here:`)
  lines.push(`- Project goals: "Winback campaign for lapsed patients."`)
  ...
  lines.push(`- Step-by-step diagram (Mermaid in a code fence) and step descriptions.`)
  lines.push(``)
  ...
  lines.push(``)
  lines.push(`Tools: \`mcp__delma__delma_add_decision\` (locked-in facts), \`mcp__delma__delma_add_action\` (open todos), \`mcp__delma__delma_supersede_decision\` (replace an old decision with a new one), \`mcp__delma__delma_complete_action\` (mark a todo done).`)
  lines.push(``)
