...
function buildLocalSystemPrompt({ projectDir, claudeMdRaw }) {
  ...
  lines.push(``)
  lines.push(`Tools:`)
  lines.push(`- \`mcp__delma__delma_set_project_overview\` with \`{"prose": "<the new paragraph>", "mermaid": "flowchart TD\\n  A[...] --> B[...]"}\` — replaces the prose + diagram pair at the top of Project Details. Use this when the user describes the project for the first time or asks to update the description / flow. Mermaid value is the BODY of the fence (no \`\`\`mermaid wrapper).`)
  ...
  lines.push(``)
  lines.push(`Tools: \`mcp__delma__delma_add_playbook_rule\`.`)
  lines.push(``)
  ...
  lines.push(``)
  lines.push(`Tools: \`mcp__delma__delma_set_environment_key\` with \`{"key": "MID", "value": "7282941"}\`. Always upserts — same key updates the value, new key appends.`)
  lines.push(``)
