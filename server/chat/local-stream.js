...
function buildLocalSystemPrompt({ projectDir, claudeMdRaw }) {
  ...
  lines.push(`</delma-suggest>`)
  lines.push(``)
  lines.push(`**"tab" must be EXACTLY one of:** \`Project Details\`, \`General Notes\`, \`File Locations and Keys\`. No other strings.`)
  lines.push(``)
  lines.push(`Each tab has a specific job. Don't bleed content across them — pick the most-correct tab even if a fact could plausibly fit two.`)
  lines.push(``)
  lines.push(`### Project Details — what's true about THIS project`)
  lines.push(`Required structure: a SHORT prose paragraph at the top describing what the project does, immediately followed by a Mermaid flowchart in a \`\`\`mermaid fence showing the data flow / system logic. Decisions, Actions, and Step descriptions appear as ### subsections below those two.`)
  lines.push(``)
  lines.push(`Goes here:`)
  lines.push(`- Project goals + description prose ("This is a winback campaign for patients who haven't engaged in 90+ days...").`)
  lines.push(`- Mermaid flowchart of the data flow / logic with boxes (\`flowchart TD\` style).`)
  lines.push(`- Defining decisions: "Lapsed = 90 days no engagement."`)
  lines.push(`- Open actions / todos: "Need to confirm the send window with Marina."`)
  lines.push(`- Per-node step descriptions in a "### Step descriptions" subsection (bullets like \`- **NodeId**: explanation\`).`)
  lines.push(``)
  lines.push(`Does NOT go here:`)
  lines.push(`- Reusable rules that apply to many projects (→ General Notes)`)
  lines.push(`- IDs, folder paths, customer keys (→ File Locations and Keys)`)
  lines.push(``)
  lines.push(`Tools:`)
  lines.push(`- \`mcp__delma__delma_set_project_overview\` with \`{"prose": "<paragraph>", "mermaid": "flowchart TD\\n  A[Step 1] --> B[Step 2]", "descriptions": {"A": "what step 1 does", "B": "what step 2 does"}}\` — replaces the prose + diagram + per-node descriptions at the top of Project Details in one shot. \`descriptions\` is optional but strongly preferred when the diagram has multiple nodes — without it, click-to-reveal modals stay empty. Mermaid value is the BODY of the fence (no \`\`\`mermaid wrapper). Description keys must match node IDs in the mermaid.`)
  lines.push(`- \`mcp__delma__delma_add_decision\` — locked-in facts.`)
  lines.push(`- \`mcp__delma__delma_add_action\` — open todos.`)
  lines.push(`- \`mcp__delma__delma_supersede_decision\` — replace an old decision with a new one.`)
  lines.push(`- \`mcp__delma__delma_complete_action\` — mark a todo done.`)
  lines.push(`- \`mcp__delma__delma_remove_decision\` / \`delma_remove_action\` — delete by text match. Use when the user wants to retract.`)
  lines.push(``)
  lines.push(`### General Notes — reusable knowledge that applies across projects`)
  lines.push(`Conventions, norms, and rules of thumb that are true across this org or BU, not specific to one project. Things you'd re-explain to a new teammate.`)
  lines.push(``)
  lines.push(`Goes here:`)
  lines.push(`- Conventions: "We always QA in the test BU before scheduling sends."`)
  lines.push(`- Naming patterns: "DEs use ENT.<Type>_<Description>."`)
  lines.push(`- Process rules: "Subject lines max 50 chars."`)
  lines.push(`- Lessons learned that should apply next time.`)
  lines.push(``)
  lines.push(`Does NOT go here:`)
  lines.push(`- Project-specific decisions (→ Project Details)`)
  lines.push(`- Identifiers, MIDs, paths (→ File Locations and Keys)`)
  lines.push(``)
  lines.push(`Tools:`)
  lines.push(`- \`mcp__delma__delma_add_playbook_rule\` — add a convention or rule.`)
  lines.push(`- \`mcp__delma__delma_remove_playbook_rule\` — delete by text match. Use when a rule no longer applies.`)
  lines.push(``)
  lines.push(`### File Locations and Keys — the boring lookup table`)
  lines.push(`Identifiers and paths the user would otherwise paste into a sticky note. Stable per BU/account, re-used across projects.`)
  lines.push(``)
  lines.push(`Goes here:`)
  lines.push(`- IDs: MID, DE customer keys, account IDs, asset IDs.`)
  lines.push(`- Paths: SFMC folder paths ("Content Builder > Emory > Reengagement"), asset URLs.`)
  lines.push(`- Field names: "ContactKey on the Subscribers DE."`)
  lines.push(`- Test resources: "Test contact email: sfmc-test@emory.edu."`)
  lines.push(``)
  lines.push(`Does NOT go here:`)
  lines.push(`- Actual passwords / OAuth secrets / API keys — those live in machine-local config files (\`~/.config/sfmc/.env\`) and must NEVER be saved to CLAUDE.md.`)
  lines.push(`- Decisions or rules (→ other tabs)`)
  lines.push(``)
  lines.push(`Tools:`)
  lines.push(`- \`mcp__delma__delma_set_environment_key\` with \`{"key": "MID", "value": "7282941"}\` — upsert. Same key updates the value, new key appends.`)
  lines.push(`- \`mcp__delma__delma_remove_environment_key\` with \`{"key": "MID"}\` — delete by exact key.`)
  lines.push(``)
  lines.push(`**Don't suggest saving facts already in CLAUDE.md above.** The full file is in your context — read it first; only propose updates that would actually change something. Don't re-propose duplicates.`)
  lines.push(``)
